...
params:
  course: 'CHE002C'
  majors: ['Biological Sciences', 'Neurobio,Physiology & Behavior', 'Biochem & Molecular Biology']
  data: '/Users/jthomas/Documents/Work Documents/SELC/Version 3 files - 6:8:23/SELC_che2c_data.csv'
  min_term: 'Fall 2018'
  max_term: 'Spring 2023'
title: "SEISMIC Equity Learning Communities Report: `r params$course` - `r params$min_term`-`r params$max_term`"
...
#cat(params$data)
dat <- read.csv(file=params$data)

```

```{r select course of interest}
#defines the course of interest using parameters
#course name must match the dataset exactly
course_of_interest <- params$course
```

```{r define majors of interest, include = FALSE}
#define the majors you are specifically interested in plotting and exploring using parameter information
majors_of_interest <- params$majors
```

```{r define minimum and maximum terms}
#defines minimum and maximum terms for the course using parameter information
min_term <- params$min_term
...
**The course data presented in this report ranges from `r min_term` to `r max_term` and includes `r length(crs_dat$st_id)` students, `r n_distinct(c(crs_dat$crs_term, crs_dat$crs_section))` section(s), and `r crs_dat$crs_instructors[1]` unique instructor(s).**
...
The plot below shows the percent of students across all terms of the course of interest that hold the following identities: declared a STEM major, transferred from another 2- or 4- year institution, are international students or are permanent residents, come from a low socioeconomic family background, are the first in their family to go to college or pursue a Bachelor's degree (first-generation, or FirstGen), are women, or come from a background historically excluded from science based on ethnicity and/or race (PEER; Asai, 2020).
...
However, the different racial and ethnic groups included in the PEER label may have different experiences in the STEM courses. These diverse experiences may be masked by the overall PEER/non-PEER categorization; certain ethnicities may experience more or less equitable outcomes. For this reason, it is also important to disaggregate our data by student race/ethnicity. Here, we consider "Asian/Asian American" as a single category; however, there is much discussion on how Asian/Asian American is not a heterogeneous group in terms of student outcomes (e.g., [Edlagan & Vaghul, 2016](https://equitablegrowth.org/how-data-disaggregation-matters-for-asian-americans-and-pacific-islanders/),[Fang,2022](https://prismreports.org/2022/05/02/disaggregation-data-justice-asian-americans/)). Importantly, none of these groups are ethnically or culturally homogeneous, and these results should be considered only a first, next step beyond the PEER/non-PEER classification.
...
::: {.center data-latex=""}
**The major(s) of interest for this report are `r pander::p(majors_of_interest, wrap = '')`.**
:::
...
-   Does this course have a mandatory distribution of grades (for example, a bell-curve or normal-distribtuion curve)? If so, is this historical distribution consistent with it?
...
Below, we can see general trends in grade distribution over time, where the distribution of grades is shown as a stacked bar plot.
...
-   *Note: If the sample size for a group in a particular term is less than `r cutoff_sections`, then that group will not be included in the plot.*
...
As with actual grades, we can also examine grade anomaly across student demographics. The following plot shows the distribution of grade anomalies for each listed student demographic. The darker portion of the distribution represents the students between the 25th and 75th percentile. The mean is a circle, with error bars showing the 95% confidence interval around the mean. Vertical lines on each plot represent the median. The dashed line across the whole plot represents the mean grade anomaly across the entire courses (all terms and all students). The solid line represents a grade anomaly of zero.
...
Please provide any feedback you may have on this report to your institutional SELC team here: [Feedback Form](YOUR%20LINK%20HERE).
...
**The SELC project is funded by National Science Foundation award [DUE-2215398](https://www.nsf.gov/awardsearch/showAward?AWD_ID=2215398&HistoricalAwards=false) and the SEISMIC collaboration is funded by the Alfred P. Sloan foundation.**
