...
synthdat %>%
  filter(crs_name == "BIO300") %>%
  mutate(crs_year = str_sub(crs_section, start = 0, end = 4),
         crs_semq = str_sub(crs_section, start = 5, end = 6)) %>%
  select(crs_name, crs_term = crs_section, crs_year, crs_semq, st_id, female, firstgen,ethniccode_cat,international,transfer,lowincomeflag, numgrade, gpao) %>%
  head() %>%
  print_html()
```
