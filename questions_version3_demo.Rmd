...
```{r course grade vs gpao, message= FALSE, warning = FALSE}
...
dat %>%
  filter(crs_name == course_of_interest) %>%
  drop_na(gpao) %>%
  #create 10 bins of GPAO and grade
  mutate(gpao_bin = cut(gpao,breaks = 10),
         grade_bin = cut(numgrade,breaks = 10)) %>%
  #get sample size for each tile
  count(gpao_bin,grade_bin) 

#re-level the bins to keep the left limit only
levels(heatmap$gpao_bin) <- sub(".(.+),(.+).", "\\1"  , levels(heatmap$gpao_bin))
levels(heatmap$grade_bin) <- sub(".(.+),(.+).", "\\1"  , levels(heatmap$grade_bin))

#plot heatmap
  ggplot(heatmap, aes(x = gpao_bin, y = grade_bin, fill = n)) +
  geom_tile() + 
...
dat %>%
  ...
  #labels
  labs(x = "Student identity", y = "Average grade anomaly", size = "N",
