...
-   [STEM major (*not currently in plots/demo dataset*)]{style="color: red;"}
...
However, the different racial and ethnic groups included in the PEER label may have different experiences in the STEM courses and this may lead to more or less equitable outcomes for these groups. For this reason, it is also important to disaggregate our data by student ethnicity. 

The following shows the specific racial/ethnicity breakdown of students in the course of interest. 
...
dat %>% 
  ...
  mutate(semq_text = recode(as.character(crs_semq), "1" = "W", "3" = "Sp", "5" = "SuI", "7" = "SuII", "10" = "F")) %>%
...
ggplot(aes(x = crs_label, y = perc, color = demo_var)) + 
  ...
  #facet_wrap(~demo_var) + 
  scale_shape_discrete(labels = c("Fall", "Spring", "Winter")) +
  theme_seismic + 
...
[*To team: what do you think of these plots to show data distribution? Useful? Not useful?*]{style="color: red;"}
...
dat <-
  dat %>%
  mutate(across(.cols = c(ethniccode_cat, firstgen, international, female, transfer, lowincomeflag), as.character)) %>%
  tidyr::replace_na(list(ethniccode_cat = "0", firstgen = "0", international = "0", female = "0", transfer = "0", lowincomeflag = "0"))
```
