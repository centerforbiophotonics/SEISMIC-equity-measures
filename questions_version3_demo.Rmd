...
The below plot shows the trends over time for each student identity (indicated by color), with points representing mean grades for all students of that identity in that term, and errorbars are the 95% confidence interval. 

```{r mean grades by identity - over time, waring = FALSE, message= FALSE}
dat %>%
...
#issue: this seems like an ugly plot :P 
```
