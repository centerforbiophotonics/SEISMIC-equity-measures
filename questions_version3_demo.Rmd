---
title: "SEISMIC Equity Learning Communities Report (DEMO)"
...
[*Note: This report is a demo created for the SELC May Institute 2023 using synthetic data.*]{style="color: blue;"}
...
library(knitr)
library(pander) #for text processing
...
dat <- read.csv(file = "~/Downloads/synthetic_data_SEISMIC-format_BIO300_2023-05-08.csv")
...
```{r grade distribution density over time, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
#overall grade distribution as a density plot over time (color = terms)
#note: this code no longer run
...
The below plot focuses on the mean grade anomaly for each of the student demographics, comparing the majority group (circles) to the historically excluded / marginalized group (triangles). Points show means, flanked by the 95% confidence interval for each group. The size of the point corresponds to the sample size of students in that group.
...
Similarly, we can also further disaggregate the PEER group into specific student races/ethnicities. The below plot shows grade anomalies by student ethnicity. Points represent mean grade anomaly, error bars the 95% confidence interval. Point size corresponds to overall sample size of students for each group.
...
dat %>%
  ...
  labs(x = "SAI", y = "Value", shape = "Gender") + 
  facet_wrap(~metric) + 
  theme_seismic
```
...
The two plots below show actual course grades and grade anomalies. In both plots, points show means and 95% confidence intervals as errorbars. Dashed lines repersent the overall course average. Color corresponds to that groups' SAI. The specific identities the student holds are indicated on the y axis; FG = First Generation, LI = Low Income family background, W = Woman, and PEER = Person from a background historically excluded based on ethnicity/race. The "All" group corresponds to students will access to all listed advantages (i.e., a non-PEER, non-Low-Income, continuing-generation man).
